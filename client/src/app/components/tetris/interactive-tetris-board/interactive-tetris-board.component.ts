...
import { BlockType } from '../immutable-tetris-models/binary-grid';
...
export class BlockData {
  ...
  constructor(
    ...
    this.svgX = (this.x-1) * (SVG_BLOCK_SIZE + SVG_BLOCK_GAP);
    this.svgY = (this.y-1) * (SVG_BLOCK_SIZE + SVG_BLOCK_GAP);
  }
}
...
export class InteractiveTetrisBoardComponent {
  ...
  readonly SVG_BOARD_WIDTH = SVG_BLOCK_SIZE + (SVG_BLOCK_SIZE + SVG_BLOCK_GAP) * 9;
  readonly SVG_BOARD_HEIGHT = SVG_BLOCK_SIZE + (SVG_BLOCK_SIZE + SVG_BLOCK_GAP) * 19;
  readonly VIEW_BOX = `0 0 ${this.SVG_BOARD_WIDTH} ${this.SVG_BOARD_HEIGHT}`;
